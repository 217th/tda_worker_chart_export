...
- Inputs: `requests[]` with `chartTemplateId`; `minImages`; `symbol`, `timeframe` from step. См. `docs-worker-chart-export/contracts/flow_run.schema.json`.
...
- External Chart API: Chart-IMG API v2, TradingView Snapshot v2 Advanced Chart (`POST https://api.chart-img.com/v2/tradingview/advanced-chart` с JSON-body и заголовком `x-api-key`); воркер не передаёт OHLCV, только `symbol`/`interval` и настройки из `chartTemplateId`; поддерживаются несколько API-ключей с per-account логированием.
