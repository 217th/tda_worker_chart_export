...
- метаданные:
  ...
  - `chartImgSymbolTemplate`: строка-шаблон TradingView‑символа (обязательное поле), например `BINANCE:{symbol}` или `BYBIT:{symbol}.P`;
...
- Поля `symbol`, `interval`, `timezone` и т.п. в реальном запросе к Chart-IMG будут формироваться воркером на основе:
  - `flow_run.scope.symbol` (базовый символ, например `BTCUSDT`)
  - `chartImgSymbolTemplate` из шаблона (обязателен): `{symbol}` подставляется в TradingView‑формат, например `BINANCE:{symbol}` → `BINANCE:BTCUSDT`
  - `flow_run.steps[stepId].timeframe`
  - глобальной конфигурации (например, дефолтный `timezone`)
- Шаблон описывает **форму и оформление графика** (style, theme, studies, overrides, drawings и т.д.) и **биржу/рынок** через `chartImgSymbolTemplate`.
